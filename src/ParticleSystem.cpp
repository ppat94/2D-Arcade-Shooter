...
void ImpulseRadialForce::setHeight(float heightMin, float heightMax) {
...
void ImpulseRadialForce::updateForce(Particle * particle) {
	...
	//
	ofVec3f dir = ofVec3f(ofRandom(-1, 1), ofRandom(heightMin, heightMax), ofRandom(-1, 1));
...
void CyclicForce::setHeight(float heightMin, float heightMax) {
    this->heightMin = heightMin;
    this->heightMax = heightMax;
}
